...
public class CartServiceImplementation implements CartSerive {
	...
	public CartItem addItemToCart(AddCartItemRequest req, String jwt) throws UserException, FoodException, CartException, CartItemException {
		...
		for (CartItem cartItem : cart.getItems()) {
			if (cartItem.getFood().equals(menuItem.get())) {
				...
				int newQuantity = cartItem.getQuantity() + req.getQuantity(); //existing + reqitem to add like bydeafult 1 + how many quanitity they want
	...
	public CartItem updateCartItemQuantity(Long cartItemId,int quantity) throws CartItemException {
		...
		cartItem.get().setTotalPrice((cartItem.get().getFood().getPrice()*quantity)); //1 price* how many they have added
